...
function onTick() 
    ...
    if not (iPBl or iPBr or isPBE or isPBD or isPB1 or isPB2 or isPB3 or isPB4 or isPB5 or isPB6 or isPB7 or isPB8 or isPB9 or isPB0) then
    	isLocked = false
    end
    ...
    if not isLocked then
	    ...
	    if isPBE then
	    	if num_s_v == "" then
	        	...
	        	output.setNumber (1, tonumber(num_s_v))
...
function onDraw()
    ...
    b_d_l_n = 1
    b_d_t_n = 0
	label_num = property.getText("label_num")
	...
	elseif num_s_v == "" then
		setC(255, 255, 255)
		screen.drawTextBox(0, 0, ws, but_s, label_num, 0, 0)
	else
		setC(255, 255, 255)
	    screen.drawTextBox(l_s + 0.5, 1, l_s_n * 4, l_s_n, num_s_v, b_d_l + 0, 0)
	end
